...
import HistorizableObjectList from "../../historizable-object/historizable-object-list.js";
...
const View = createVisualComponent({
  ...
  render(props) {
    ...
    const { dataObject: awscDto } = UuArtifact.useAwsc();
    ...
    if (pageDto?.handlerMap?.load) {
      ...
      pageDto.handlerMap.load = () => origPageLoad({ documentOid: awscDto.data?.data.uuEccData.documentOid });
    }
    ...
    return (
      <Uu5Elements.SpacingProvider type="tight">
        <Uu5Elements.Grid
          ...
          templateColumns={{ xs: "1fr", m: "360px 1fr" }}
        ...
        >
          <Uu5Elements.Grid.Item gridArea="topSection">
            <PageTopSection
              uuAppType="uuDevKitUuAppTemplate"
              schema="uuDevKitUuAppTemplate"
              uuType="uu-devkituuapptemplate-maing01"
              historyType="none"
              useUuAppWorkspace={true}
            />
