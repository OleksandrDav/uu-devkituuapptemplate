...
import UuEbc from "uu_editablebinarycontentg02";
...
function getActionList({ onOpenAttachments } = {}) {
  let actionList = [];
...
const InternalPageTopSection = createVisualComponent({
  ...
  render(props) {
    ...
    const entityData = artifactDto?.data?.data;
    ...
    const [attachmentsModalProps, openAttachmentsModal] = Uu5Elements.useModal();

    // FIXME solve properly creating of all necessary providers if necessary
    const actionList = useMemo(
      () => getActionList({ onOpenAttachments: openAttachmentsModal }),
    ...
    return (
      <>
        <Uu5ComponentKitCore.RouteHeader
          ...
          type="tabs"
          itemList={
            pageList.length > 1 ? pageList.map((page) => ({ label: page.name, code: page.pageOid })) : undefined
          }
        ...
        <Uu5Elements.Modal {...attachmentsModalProps} header="Attachments" size="l">
          {attachmentsModalProps.open ? (
            <UuEbc.File.List
              baseUri={baseUri}
              collectionOid={
                entityData?.uuEbcData?.uuBbAttachmentsCollectionOid ||
                entityData?.uuEbcData?.uuBBAttachmentsOid ||
                entityData?.uuEbcData?.collectionOid
              }
              oid={entityData?.oid}
              entityType={props.schema}
              uuType={props.uuType}
              historyType={props.historyType}
              useUuAppWorkspace={props.useUuAppWorkspace}
              useAppWorkspaceProvider={props.useUuAppWorkspace}
